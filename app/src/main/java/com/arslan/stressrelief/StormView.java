...
/** Hosts the storm and turns touches into strike targets. */
final class StormView extends GLSurfaceView {

    private final StormRenderer renderer;

    StormView(Context context) {
        super(context);
        setEGLContextClientVersion(3);
        setEGLConfigChooser(8, 8, 8, 0, 0, 0);
        renderer = new StormRenderer(context);
        setRenderer(renderer);
        setRenderMode(RENDERMODE_CONTINUOUSLY);
        setPreserveEGLContextOnPause(true);
    }

    StormRenderer getStormRenderer() {
        return renderer;
    }
